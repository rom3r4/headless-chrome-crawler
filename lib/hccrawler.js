...
Puppeteer.use(require('puppeteer-extra-plugin-anonymize-ua')({
  makeWindows: (process.platform === 'win32'),
}));

const GenuinePuppeteer = require('puppeteer');
...
const deviceNames = Object.keys(GenuinePuppeteer.devices);
...
class HCCrawler extends EventEmitter {
  ...
  static async connect(options) {
    const puppeteerOptions = pick(options, CONNECT_OPTIONS);
    // https://developer.chrome.com/articles/new-headless/
    if (puppeteerOptions.headless === undefined || puppeteerOptions.headless) {
  ...
  static async launch(options) {
    // https://developer.chrome.com/articles/new-headless/
    const puppeteerOptions = pick(options, LAUNCH_OPTIONS);
    if (puppeteerOptions.headless === undefined || puppeteerOptions.headless) {
      puppeteerOptions.headless = 'new';
    }
  ...
  async queue(options) {
    await Promise.all(
      map(isArray(options) ? options : [options], async (_options) => {
        ...
        const skip = await this._skipRequest(queueOptions);
        if (skip) {
  ...
  async _push(options, depth, previousUrl) {
    ...
    this._queue.push(options, depth, previousUrl, priority);
  }
  ...
  /**
   * Returns true if the domain passed in options.url is allowed in options.allowedDomains or not
   * forbidden in options.deniedDomains
   *
   * @param {!Object} options
  ...
  async _skipRequest(options) {
    ...
    const allowedPath = this._checkAllowedPaths(options);
  ...
  async _getUserAgent(options) {
    ...
    if (GenuinePuppeteer.devices[options.device]) {
      return GenuinePuppeteer.devices[options.device].userAgent;
    }
  ...
  _checkAllowedDomains(options) {
    if (options.allowedDomains === undefined) return false;
  ...
  _checkAllowedPaths(options) {
    const { pathname } = parse(options.url);
    if (options.deniedPaths && Helper.checkPathMatch(options.deniedPaths, pathname)) {
      return false;
    }
    if (options.allowedPaths && !Helper.checkPathMatch(options.allowedPaths, pathname)) {
      return false;
    }
    return true;
  }
